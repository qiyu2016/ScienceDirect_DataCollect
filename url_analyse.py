    ...
    def analyse_EveryPage(self, new_url, count):
        ...
        while flag<20:
            try:
                ...
                soup = BeautifulSoup(resp, "html.parser")
                try:
                    print('论文页>正在从网页解析内容')      # 运行到这一步，，，论文页>正在从网友解析内容  ---论文页>暂停 3.36 秒爬下一篇  --报错： [由于连接放在一段时间后没有正确答复或连接的主机没有反应，连接尝试失败。]
                    
                    title = soup.find("span", {"class": "title-text"}).get_text()
                    temp = soup.find("div", {"class": "abstract author"})
#                    abstract = temp.find("p").get_text()
                    abstract=""
                    for tempall in temp.find_all("p"):
#                      abstag = tempall.find_all("p")
                      abstract = abstract+" "+tempall.get_text()
                except:
                    print('论文页>从网页解析内容失败')
                    title = '论文页>解析失败'
                    abstract = '论文页>解析失败'
        ...
        # 休息一会儿再爬
        time_lapse = random.uniform(7, 15)
